...
internal object Updater {
	...
	fun updateSQL(db: SQLiteInterface, oldVersion: Int) {
        ...
        if(oldVersion <= 53) {
            db.execSQL("ALTER TABLE questionnaires ADD COLUMN script_filter TEXT DEFAULT '';")
        }

        if(oldVersion <= 55) {
            db.execSQL("ALTER TABLE studies ADD COLUMN additionalDaysActive INTEGER DEFAULT 0;")
        }

        if(oldVersion <= 56) {
            db.execSQL("ALTER TABLE studies ADD COLUMN legacyScheduling INTEGER DEFAULT 0;")
            db.execSQL("UPDATE studies SET legacyScheduling=1;")
            db.execSQL("ALTER TABLE schedules ADD COLUMN startDayOne INTEGER DEFAULT 1;")
            db.execSQL("UPDATE schedules SET startDayOne=0;")
        }
		...
		if(oldVersion <= 59) {
			db.execSQL("ALTER TABLE studies ADD COLUMN salt TEXT DEFAULT '';")
		}
    }
